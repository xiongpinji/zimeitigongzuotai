...
**当前状态：阶段一方案已获用户确认，产品开发正在推进；用户已撤销百炼 Credits 硬性上限。** `app/` 已导入固定提交的 Lingji Cut 桌面源码基线。后续多账号、直播高光、智能合成及批量发布能力仍按验收矩阵逐项实现；不能把路线图或第三方 README 声明视为真实账号验收。
...
- [Lingji Cut 来源与许可](docs/third-party/lingji-cut.md)
- [Windows 源码基线验证](docs/validation/p0-1-windows.md)

已确认的方案：以 [Lingji Cut](https://github.com/yoqu/lingji-cut) 为桌面编辑与发布基座，按功能组合 [Easel](https://github.com/ZJU-REAL/Easel) Skills、[HotClip](https://github.com/xixihhhh/hotclip) CLI/MCP 候选、[OpenCut-AI](https://github.com/GinoongFlores/OpenCut-AI) 语义索引路线和本项目的持久任务引擎。组件可选择性移植源码或以独立进程接入，统一通过版本化契约和本地任务 API 协作。开发 Agent 按 [编排约定](docs/orchestration.md) 启动。

Windows 本地开发从 `app/` 开始：`npm ci` 安装锁定依赖，`npm test` 跑自动化测试，`npm run build` 构建桌面代码，`npm run dev` 启动开发版。安装和构建的实际基线结果在上面的 Windows 验证记录中；执行开发启动命令不等于已完成人工编辑与导出验收。
